...
const useAuthStore = /* @__PURE__ */ defineStore("auth", () => {
  ...
  async function login(email, password) {
    ...
    error.value = null;
    try {
      const response = await sendMessage$1("AUTH_LOGIN", { email, password });
      if (response.success) {
  ...
  async function register(email, password, username) {
    ...
    error.value = null;
    try {
      const response = await sendMessage$1("AUTH_REGISTER", { email, password, username });
      if (response.success) {
        user.value = response.data.user;
        isAuthenticated.value = true;
        return response.data;
  ...
  async function logout() {
    ...
    error.value = null;
    try {
      const response = await sendMessage$1("AUTH_LOGOUT");
      if (response.success) {
        user.value = null;
  ...
  async function checkSession() {
    isLoading.value = true;
    try {
      const response = await sendMessage$1("AUTH_GET_CURRENT_USER");
...
const useCommunitiesStore = /* @__PURE__ */ defineStore("communities", () => {
  const allCommunities = /* @__PURE__ */ ref([]);
  const selectedCommunityId = /* @__PURE__ */ ref(null);
  const isLoading = /* @__PURE__ */ ref(false);
  const error = /* @__PURE__ */ ref(null);
